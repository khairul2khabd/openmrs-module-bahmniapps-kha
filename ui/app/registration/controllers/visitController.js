...
angular.module('bahmni.registration')
    .controller('VisitController', ['$window', '$scope', '$rootScope', 'registrationCardPrinter', '$state', '$bahmniCookieStore', 'patientService', 'encounterService', '$stateParams', 'spinner', '$timeout', '$q', 'appService', 'openmrsPatientMapper', 'contextChangeHandler', 'messagingService', 'sessionService', 'visitService', '$location', '$translate',
        'auditLogService', 'formService',
        function ($window, $scope, $rootScope, registrationCardPrinter, $state, $bahmniCookieStore, patientService, encounterService, $stateParams, spinner, $timeout, $q, appService, openmrsPatientMapper, contextChangeHandler, messagingService, sessionService, visitService, $location, $translate, auditLogService, formService) {
            var vm = this;
            ...
            var afterSave = function (s) {
                var forwardUrl = appService.getAppDescriptor().getConfigValue("afterVisitSaveForwardUrl");
                var isPrint = appService.getAppDescriptor().getConfigValue("afterSavePrint");
                if (s === "print" && isPrint != null && isPrint.print === true) {
                    $scope.print();
                }
                if (forwardUrl != null) {
            ...
            $scope.submit = function (sts) {
                return validate().then(save).then(afterSave(sts));
            };
